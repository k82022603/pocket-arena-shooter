...
const KEY = (id: CharacterId) => `arena.loadout.${id}`;
...
export function selectedLoadout(scene: Phaser.Scene, id: CharacterId): WeaponKind {
  ...
  if (fromRegistry !== undefined && options.includes(fromRegistry)) return fromRegistry;
  try {
    const stored = Number(localStorage.getItem(KEY(id)));
    if (options.includes(stored as WeaponKind)) return stored as WeaponKind;
  } catch {
    /* 저장 불가 환경 */
  }
  return options[0]!;
...
export function setLoadout(scene: Phaser.Scene, id: CharacterId, weapon: WeaponKind): void {
  if (!isLoadoutWeapon(weapon) || !LOADOUTS[id].includes(weapon)) return;
