...
- 대전 동기화 (M2, 호스트 권위): 호스트가 양쪽 입력으로 시뮬레이션을 돌리고 20Hz로 전체 스냅샷 전송. 게스트는 자기 캐릭터를 예측하고 스냅샷 ack 기준으로 재조정, 상대·탄환은 100ms 지연 보간. 게스트 입력은 틱 번호가 붙어 호스트 큐(최대 100ms)에서 순서대로 소비
- 아직 없음: 발사 예측(게스트 탄환은 1 RTT 뒤에 보임), 되감기 판정, 보정 스무딩, 재접속 스냅샷 복구
