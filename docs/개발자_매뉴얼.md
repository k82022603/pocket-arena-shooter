...
- **분량**: 소스 48개 파일 6,433줄 (TypeScript 중심). 아래 줄 수는 모두 `wc -l` 기준이며, `find apps packages scripts -name "*.ts" -o -name "*.mts" -o -name "*.mjs"` 로 다시 셀 수 있습니다
...
**대가**: JS 번들 1,726KB, gzip 428KB (프로덕션 빌드 실측). 첫 로딩이 Wi‑Fi에서 1~2초 걸립니다. 트리셰이킹이 잘 안 되는 구조라 줄이기 어렵습니다.
...
`ArenaScene`은 이 인터페이스만 봅니다. 그래서 씬 코드 한 벌로 솔로·호스트·게스트를 모두 처리합니다. 구현체는 `SoloSync`(53줄), `HostSync`(124줄), `GuestSync`(329줄) — 줄 수 차이가 곧 난이도 차이입니다.
...
| 적 | HP | 이동 | 목표 | 행동 |
|----|----|------|------|------|
| 척후병 | 30 | 150px/s | 가장 가까운 생존 플레이어 | 직진, 접촉 피해 6 (0.75초 간격), 반지름 14 |
| 포수 | 50 | 120px/s | 가장 가까운 플레이어 | 거리 320 유지, 1.5초마다 사격(피해 8), 사거리 600, 접촉 피해 5, 반지름 16 |
| 강습병 | 200 | 70px/s | **코어** | 플레이어 무시, 접촉 피해 20, 반지름 26 |
...
| 조정 | 값 | 근거 |
|------|-----|------|
| `SOLO_ENEMY_SCALE` | 0.45 | 배율을 훑어보니 0.5에서 0.45로 내릴 때 클리어율이 급격히 오르고, 0.35로 더 내려도 이득이 없었다. 즉 이 구간이 플레이어 화력과 스폰 속도가 만나는 문턱이다 |
...
apps/web/src/
  ...
  sim/                       ★ 의존성 없는 순수 시뮬레이션
    ...
    core.ts      (373)       틱 진행, 이동·발사·판정·픽업, 승패
    coop.ts      (252)       웨이브·적 AI·코어·부활
    bot.ts       (104)       1인 대전 상대 AI
    weapons.ts   ( 77)       무기·픽업 정의와 파티마별 선택지
    characters.ts( 61)       파티마 능력치
    serialize.ts (360)       바이너리 인코딩/디코딩 전부
    history.ts   ( 85)       되감기 판정용 위치 링 버퍼 (플레이어 + 적)
  ...
  net/                       ★ 전송 계층
    ...
    session.ts   (272)       SessionLink: 끊김 감지·재접속·릴레이 합의
  ...
  game/                      ★ Phaser 계층
    ...
    sync/                    GameSync(32) + Solo(53)/Host(124)/Guest(329) 구현
    ...
    record.ts    ( 42)       로컬 전적
    ui.ts        ( 35)       버튼·라벨 헬퍼
    outcomeText.ts( 34)      결과 화면 문구 결정 (Phaser 없음, 헤드리스 검사 대상)
...
scripts/
  ...
  sim-check.ts   (408)       헤드리스 규칙 검사 35개
  coop-balance.ts(138)       협동 난이도 측정 (자동 플레이, npm run check:balance)
  check-docs.mjs ( 95)       문서 수치와 코드 대조 (npm run check:docs)
  make-icons.mjs (107)       의존성 없는 PNG 아이콘 생성
...
화면 없이 순수 로직만 Node에서 돌려 규칙을 숫자로 확인합니다(현재 35개 항목).
...
PASS  적 유탄이 코어를 깎는다
PASS  적이 남으면 다음 웨이브로 넘어가지 않는다
PASS  10웨이브 전부 소탕하면 승리
PASS  코어 파괴 또는 전원 다운이면 패배
PASS  혼자 하기는 다운이 곧 패배
PASS  다운된 아군 곁 2초면 절반 HP로 부활
PASS  아군과 코어는 내 탄에 맞지 않는다
PASS  부활 진행도는 이탈하면 2배로 감소
PASS  1인 편성은 2인보다 적다
PASS  1인 방어는 픽업이 9초마다 나온다
PASS  씬 전환 중 도착한 캐릭터 선택이 보존된다
PASS  코어가 남아 있으면 전멸로 표시
PASS  코어가 0이면 코어 함락으로 표시
PASS  1인 방어는 전투 불능으로 표시
PASS  연결이 끊겨 끝나면 방어 중단으로 표시
PASS  클리어는 방어 성공으로 표시
PASS  적 대상 되감기: 지나간 자리에 쏜 탄이 명중
PASS  되감기 없이는 같은 탄이 빗나감
```
...
터치 감각, 실제 fps, 소리, 카메라 QR, 실제 Wi‑Fi 지연은 반드시 폰에서 확인해야 합니다. 무엇을 어떻게 보고 합격 기준이 무엇인지는 [실기 테스트 가이드](실기_테스트_가이드.md)에 체크리스트로 있습니다. 세 단계의 개념 구분은 [테스트 방식과 헤드리스](테스트_방식과_헤드리스.md).
...
| 번들 크기 | JS 1,726KB, gzip 428KB (Phaser) | PixiJS로 교체하면 크게 줄지만 씬·입력·오디오 재구현 필요 |
...
- [실기 테스트 가이드](실기_테스트_가이드.md) — 폰에서 확인할 항목과 합격 기준, 진단 줄 읽는 법
- [캐릭터 설정](캐릭터_설정.md)
