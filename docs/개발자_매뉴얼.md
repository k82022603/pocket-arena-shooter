...
`ArenaScene`은 이 인터페이스만 봅니다. 그래서 씬 코드 한 벌로 솔로·호스트·게스트를 모두 처리합니다. 구현체는 `SoloSync`(59줄), `HostSync`(131줄), `GuestSync`(362줄) — 줄 수 차이가 곧 난이도 차이입니다.
...
| `ArenaScene` | 952 | 입력·틱 루프·렌더·연출·HUD — 가장 큰 파일 |
...
apps/web/src/
  ...
  sim/                       ★ 의존성 없는 순수 시뮬레이션
    ...
    core.ts    (393)       틱 진행, 이동·발사·판정·픽업, 승패
    coop.ts    (257)       웨이브·적 AI·코어·부활
    ...
    events.ts    ( 27)       피해 주체 등 시뮬레이션 사건 정의
  net/                       ★ 전송 계층
  ...
  game/                      ★ Phaser 계층
    ...
    sync/                    GameSync(35) + Solo(59)/Host(131)/Guest(362) 구현
    ...
    record.ts    ( 42)       로컬 전적
    ui.ts        ( 35)       버튼·라벨 헬퍼
    recorder.ts  (167)       경기 진단 기록과 내보내기 (Phaser 없음)
...
scripts/
  ...
  sim-check.ts (627)       헤드리스 규칙 검사 44개
...
화면 없이 순수 로직만 Node에서 돌려 규칙을 숫자로 확인합니다(현재 44개 항목).
