...
packages/protocol  양쪽이 공유하는 메시지 타입 (36줄)
...
`ArenaScene`은 이 인터페이스만 봅니다. 그래서 씬 코드 한 벌로 솔로·호스트·게스트를 모두 처리합니다. 구현체는 `SoloSync`(44줄), `HostSync`(106줄), `GuestSync`(232줄) — 줄 수 차이가 곧 난이도 차이입니다.
...
적 AI는 상태 기계 없이 **매 틱 목표를 다시 계산**하는 단순 방식입니다(27줄 수준). 유지보수가 쉽고 결정적입니다.
...
`sim/history.ts`는 최근 64틱의 위치를 링 버퍼에 저장합니다(34줄). 상한 200ms를 두는 이유: 이보다 길면 "이미 엄폐한 나를 맞혔다"는 불만이 생깁니다. 모든 대전 게임이 이 트레이드오프를 다룹니다.
...
| 씬 | 줄 수 | 역할 |
|----|-------|------|
| `BootScene` | 20 | 이미지 프리로드, QR 초대 코드 회수 |
| `TitleScene` | 191 | 파티마·무기·모드 선택, 초상 카드, 초대 안내 |
| `LobbyScene` | 163 | 방 만들기(QR 생성), 참가(스캔/코드), 연결 상태 |
| `ArenaScene` | 748 | 입력·틱 루프·렌더·연출·HUD — 가장 큰 파일 |
| `ResultScene` | 168 | 통계표, 로컬 전적, 재경기 핸드셰이크 |
...
- `VirtualStick`(80줄): 터치 시작점에 생성되는 플로팅 스틱. 데드존 15%, 최대 반경 60px
- `DesktopControls`(86줄): 키보드·마우스. **키나 마우스를 쓰는 순간 활성화**되고 터치를 쓰면 비활성화. 활성 시 스틱을 `touchOnly`로 바꿔 마우스 클릭이 스틱을 잡지 않게 함
...
`Fx`(130줄)는 파티클·링·잔상·총구 화염을 관리하며 `quality` 배율로 저사양 기기에서 수를 줄입니다.

### 8.4 캐릭터를 코드로 그리기 (`game/fx/Fatima.ts`, 285줄)
...
### 8.5 초상 이미지 배경 제거 (`game/fx/Portraits.ts`, 123줄)
...
apps/web/src/
  ...
  sim/                       ★ 의존성 없는 순수 시뮬레이션
    types.ts     (162)       상태 타입과 모든 상수
    core.ts      (336)       틱 진행, 이동·발사·판정·픽업, 승패
    coop.ts      (227)       웨이브·적 AI·코어·부활
    bot.ts       ( 89)       1인 대전 상대 AI
    weapons.ts   ( 64)       무기·픽업 정의와 파티마별 선택지
    characters.ts( 54)       파티마 능력치
    serialize.ts (337)       바이너리 인코딩/디코딩 전부
    history.ts   ( 34)       되감기 판정용 위치 링 버퍼
    prng.ts      ( 27)       xorshift32
  net/                       ★ 전송 계층
    transport.ts ( 11)       Transport 인터페이스 (교체 지점)
    webrtc.ts    (157)       DataChannel 2개, ICE, RTT 핑
    relay.ts     ( 41)       WebSocket 릴레이 폴백
    signaling.ts ( 92)       시그널링 클라이언트
    session.ts   (228)       SessionLink: 끊김 감지·재접속·릴레이 합의
    connect.ts   ( 17)       방 만들기/참가 진입점
    pairing.ts   ( 69)       QR 링크 생성·파싱, 초대 코드 보관
    simulated.ts ( 52)       지연·손실 주입 래퍼 (테스트용)
  game/                      ★ Phaser 계층
    scenes/                  Boot·Title·Lobby·Arena·Result
    sync/                    GameSync 인터페이스 + Solo/Host/Guest 구현
    input/                   VirtualStick, DesktopControls
    fx/                      Fx(파티클), Fatima(캐릭터), Portraits(초상)
    audio/Sfx.ts (157)       Web Audio 합성 효과음
    loadout.ts   ( 26)       무기 선택 저장
    record.ts    ( 38)       로컬 전적
    ui.ts        ( 32)       버튼·라벨 헬퍼
  ui/overlay.ts  (123)       DOM 오버레이: 코드 입력, QR 스캐너, 토스트

apps/signaling/src/index.ts  (154)  시그널링 서버 전체
packages/protocol/src/index.ts (36) 공유 메시지 타입
scripts/
  start-lan.mjs  ( 43)       서버 2개 실행 + LAN 주소로 브라우저 열기
  lan-ip.mjs     ( 17)       가상 어댑터 걸러낸 LAN IPv4 탐색
  sim-check.ts   ( 78)       헤드리스 규칙 검사 14개
...
화면 없이 `sim/`만 Node에서 돌려 규칙을 숫자로 확인합니다(현재 14개 항목).
