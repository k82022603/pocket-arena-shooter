...
`ArenaScene`은 이 인터페이스만 봅니다. 그래서 씬 코드 한 벌로 솔로·호스트·게스트를 모두 처리합니다. 구현체는 `SoloSync`(53줄), `HostSync`(124줄), `GuestSync`(354줄) — 줄 수 차이가 곧 난이도 차이입니다.
...
| `ArenaScene` | 909 | 입력·틱 루프·렌더·연출·HUD — 가장 큰 파일 |
| `ResultScene` | 203 | 통계표, 로컬 전적, 재경기 핸드셰이크 |
...
apps/web/src/
  ...
  sim/                       ★ 의존성 없는 순수 시뮬레이션
    ...
    core.ts      (375)       틱 진행, 이동·발사·판정·픽업, 승패
  ...
  game/                      ★ Phaser 계층
    ...
    sync/                    GameSync(32) + Solo(53)/Host(124)/Guest(354) 구현
    ...
    record.ts    ( 42)       로컬 전적
    ui.ts        ( 35)       버튼·라벨 헬퍼
    recorder.ts  (123)       경기 진단 기록과 내보내기 (Phaser 없음)
...
scripts/
  ...
  sim-check.ts   (567)       헤드리스 규칙 검사 41개
...
화면 없이 순수 로직만 Node에서 돌려 규칙을 숫자로 확인합니다(현재 41개 항목).
...
PASS  건강한 호스트에서는 게스트 이동이 그대로 반영된다  (버린 입력 0, 권위 이동 438px, 화면 차이 7px)
